...
class GA():
    ...
    def objective_function(self, individual, exp_name):
        ...
        model_train = train_audio_model( self.data_path, self.file_path, self.num_classes, k_shots, batch_size, num_epochs, lr, momentum, weight_decay,
                                        temperature,temperature_closs, alpha)
        path_best_model, _ = model_train.train(exp_name)

        loaded_model = load(path_best_model)
        metrics=self.test(individual, loaded_model)
        best_f1score=metrics['f1_macro']
    ...
    def GA(self, population_size=10, generations=20, elite_size=5, mutation_rate=0.1):
        ...
        print(f"Global best path: {global_model_params_path}")

        with open('Best/best_ind_gen.json', 'w') as f:
            json.dump(best_ind_gen, f, indent=4)

        # Test the model with the best hyperparameters
        loaded_model = load(global_model_params_path)
        metrics=self.test(global_best_individual, loaded_model)
...
file_path = 'Biolingual_audio_text_embeddings2.pt'
